...
public class DdtXorPlusThreadExtended extends TableThread {

    public DdtXorPlusThreadExtended(int[][] lat, int startAlpha, int endAlpha, int basis) {
    ...
    public void run() {
        for (int alpha = startAlpha; alpha < endAlpha; alpha++) {
            for (Integer a : SboxExtender.getExtentionParam()) {
                ...
                for (int b = 0; b < Q; b++) {
                    for (int x = 0; x < Q; x++) {
                        int out = (allExponents[basis][((a * x + b) & 0xFF) ^ alpha] - allExponents[basis][(a * x + b) & 0xFF] + Q) & 0xFF;
